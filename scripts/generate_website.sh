...
function generate_webpage () {
    ...
    local pokemon_template="$5"

    local sidebar=$( generate_sidebar "${pokemon_types[*]}" "$selected_type" )
    local pokemon_cards=$( generate_cards "${pokemon_data}" "$selected_type" "${card_template}")
...
function generate_website () {
    ...
    do
        ...
        generate_webpage "${pokemon_data}" "${pokemon_types[*]}" "$type" "${card_template}" "${pokemon_template}" > $directory/${type}.html
    done
}